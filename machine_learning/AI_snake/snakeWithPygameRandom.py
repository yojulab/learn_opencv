...
class Snake:
    ...
    def draw(self, screen):
        pygame.draw.rect(screen, pygame.color.Color(
            "yellow"), (self.stack[0].x, self.stack[0].y, SNAKE_SIZE, SNAKE_SIZE), 0)
        counter = 1
        while(counter < len(self.stack)):
            if(self.stack[counter].color == "NULL"):
                ...
                continue
            pygame.draw.rect(screen, pygame.color.Color(
                "white"), (self.stack[counter].x, self.stack[counter].y, SNAKE_SIZE, SNAKE_SIZE), 0)
            counter += 1
...
# add some code for me
def getAutoKeypress(gameTime):
    returnKey = None
    if (gameTime % 50) <= 0:
        # returnKey = random.choice(KEY)
        returnKey = random.randrange(1,4)
    return returnKey
...
def main():
    ...
    while(endgame != 1):
        ...
        keyPress = getAutoKeypress(gameTime)
        # Position Update
        if(keyPress):
